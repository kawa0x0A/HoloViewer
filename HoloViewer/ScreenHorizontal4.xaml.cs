...
public partial class ScreenHorizontal4 : ContentPage
{
    public ScreenHorizontal4()
    {
        InitializeComponent();
    ...
    private async Task<Rect[]> CalcScreenCapturePosition()
    {
        ...
        {
            var x = 0;
            var y = captureToolbar.BindingModelView.IsCapture[0] ? captureToolbar.BindingModelView.IsCaptureYoutubePlayerOnly ? await ScreenCapture.GetYoutubePlayerHeight(captureToolbar.CaptureTargetWebviews[0]) + await ScreenCapture.GetYoutubePlayerHeight(captureToolbar.CaptureTargetWebviews[1]) : ScreenCapture.GetWebViewHeight(captureToolbar.CaptureTargetWebviews[0]) + ScreenCapture.GetWebViewHeight(captureToolbar.CaptureTargetWebviews[1]) : captureToolbar.BindingModelView.IsCapture[1] ? captureToolbar.BindingModelView.IsCaptureYoutubePlayerOnly ? await ScreenCapture.GetYoutubePlayerHeight(captureToolbar.CaptureTargetWebviews[1]) : ScreenCapture.GetWebViewHeight(captureToolbar.CaptureTargetWebviews[1]) : captureToolbar.BindingModelView.IsCapture[2] ? captureToolbar.BindingModelView.IsCaptureYoutubePlayerOnly ? await ScreenCapture.GetYoutubePlayerHeight(captureToolbar.CaptureTargetWebviews[2]) : ScreenCapture.GetWebViewHeight(captureToolbar.CaptureTargetWebviews[2]) : 0;
